...
#sample_size = 256
param_values_ABA_features = SALib.sample.morris.sample(problem_ABA_features, 32,local_optimization = True, num_levels=4)
...
x_test_satellisample.replace([np.inf, -np.inf], np.nan, inplace=True)
x_test_satellisample
...
x_test_satellisample.fillna(x_test_satellisample.median(), inplace=True)
...
for x in range(300):
    ...
    pred_y = (model.predict(x_test_satelliscaled) > 0.5).astype("float64")
    print("testing ends")
    ...
    print("SA analysis")
    Si = morris.analyze(problem_ABA_features,x_test_satelliscaled, pred_y_array, print_to_console=False)
...
resultdf.to_csv("265Features_morris.csv")
